...
async def api_create_setting(setting: LprSettingCreate, db: AsyncSession = Depends(get_db), current_user: UserInDB=Depends(get_admin_or_staff_user)):
...
async def api_update_setting(setting_id: int, setting: LprSettingUpdate, db: AsyncSession = Depends(get_db), current_user: UserInDB=Depends(get_admin_or_staff_user)):
    setting_op = LprSettingOperation(db)
